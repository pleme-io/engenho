...
| Release path | release.yml has never succeeded (ghcr 403; the aarch64 openssl-sys build fails). Substrate's rust-auto-release runs `bump`, which tags and pushes, in parallel with `test`. `ship` needs both jobs, but it is skipped for private repos. So engenho's tag, and the release.yml run it triggers, never wait for the test gate. On 2026-09-18, `bump` and the test gate for v0.53.117 both started at 21:21:51; `bump` pushed the tag and its release.yml run started at 21:27:21. The engenho-mcp linux-x86_64 leg published v0.53.117 at 21:33:49, 6m28s after that run started. The test gate had gone red at 21:33:33, 16 s before the publish, and nothing connected the two. In rust-binary-release's default mode, the two calls in one release.yml run (engenho-mcp and engenho-cluster-config-render) both upload a workflow artifact named `linux-x86_64-binary`. | rust-auto-release.yml:161-218, :258-420; rust-binary-release.yml:527-530 at substrate 1eb48fe (read); auto-release run 35396380114, release run 35396851404, release API `published_at` (run) |
...
- **"v0.53.117 was published 16 s after the tag"** (an earlier version of the release-path row) is wrong. It was published 6m28s after its release run started; the 16 s separates the red test gate from the publish.
