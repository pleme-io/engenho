...
pub fn instantiate(decl: &FabricFace) -> Result<Box<dyn Face>, FaceError> {
    match &decl.kind {
        ...
        FaceKind::Nomad { .. } => Err(FaceError::Unsupported(
            "NomadFace impl lands at engenho-revoada R6+".into(),
        )),
...
mod tests {
    ...
    #[test]
    fn instantiate_nomad_returns_unsupported_until_r6() {
        let decl = FabricFace {
            name: "nomad-test".into(),
            kind: FaceKind::Nomad {
                version: "1.7".into(),
            },
        };
        // `Box<dyn Face>` doesn't impl Debug, so we pattern-match
        // the Result rather than calling `.unwrap_err()`.
        match instantiate(&decl) {
            Err(FaceError::Unsupported(msg)) => {
                assert!(msg.contains("R6"), "msg: {msg}");
            }
            Err(other) => panic!("expected Unsupported, got {other:?}"),
            Ok(_) => panic!("expected Err for Nomad face, got Ok"),
        }
    }
