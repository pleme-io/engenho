...
impl BootConfig {
    ...
    pub(crate) fn read(config: &EngenhoConfig) -> Result<Self, Unhonoured> {
        ...
        let not_run = [
            ...
            read_fabric(fabric, legacy_teia.as_ref()),
        ]
        .into_iter()
        .flatten()
        .collect();
...
/// `fabric` + the retired `teia` section: what runs, and what is only read.
///
/// `Fabric::InBinary` IS what this runtime does — the store's raft transport
/// is in-process (`boot_store`) — so it records nothing. A `teia:` section
/// still in the file is accepted for one release and read by nothing (I11,
/// T5.2); that is the not-run record.
fn read_fabric(fabric: &Fabric, legacy_teia: Option<&LegacyTeiaSection>) -> Option<NotRun> {
    match fabric {
        Fabric::InBinary => {}
    }
    legacy_teia.map(|_| NotRun::Teia)
}
...
pub(crate) enum NotRun {
    ...
    /// A retired `teia:` section is still in the config. It is accepted so
    /// the node boots, and read by nothing: the store's raft transport is
    /// in-process.
    Teia,
}
...
mod tests {
    ...
    /// run: the region and the formation. It carries no retired `teia:`
    /// section, so there is nothing to record for the fabric (T5.2).
    #[test]
    ...
    /// A config still carrying the retired `teia:` section boots, and the
    /// section is recorded as read-and-not-run (T5.2 accepts it for one
    /// release; nothing opens a NATS connection).
    #[test]
    fn a_retired_teia_section_is_accepted_and_recorded() {
        let boot = read(|c| {
            c.legacy_teia = serde_json::from_value(serde_json::json!({
                "servers": ["nats://127.0.0.1:4222"],
                "cluster": "engenho",
            }))
            .expect("the retired section still deserializes");
        })
        .expect("a config with the retired section still boots");
        assert!(boot.not_run.contains(&NotRun::Teia), "{:?}", boot.not_run);
    }
