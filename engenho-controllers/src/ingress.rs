...
impl TraefikIngressBackend {
    ...
    /// `Path`/`PathPrefix`); the YAML document itself is built as a
    /// typed [`engenho_types::egress::TraefikIngressRoute`] + rendered
    /// through `serde_yaml` (★★ TYPED EMISSION — no `format!()` of
    /// YAML syntax).
    #[must_use]
    pub fn render_crd(route: &IngressRoute) -> String {
        use engenho_types::egress::TraefikIngressRoute;
        ...
        TraefikIngressRoute::new(
            sanitize_name(&route.ingress_id, &route.host, &route.path),
            match_rule,
            route.backend_service.clone(),
            route.backend_port,
            route.tls_secret.clone(),
        )
        .to_yaml()
    }
